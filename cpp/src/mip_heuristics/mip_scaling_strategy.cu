...
template <typename i_t, typename f_t>
void compute_row_inf_norm_with_cub(const problem_t<i_t, f_t>& op_problem,
...
template <typename i_t, typename f_t>
void compute_big_m_skip_rows_with_cub(const problem_t<i_t, f_t>& op_problem,
                                      rmm::device_uvector<std::uint8_t>& temp_storage,
                                      size_t temp_storage_bytes,
                                      rmm::device_uvector<f_t>& row_inf_norm,
...
void mip_scaling_strategy_t<i_t, f_t>::scale_problem()
{
  ...
  if (nnz > 0) {
    auto coeff_abs_iter = thrust::make_transform_iterator(op_problem_scaled_.coefficients.data(),
    ...
    RAFT_CUDA_TRY(cub::DeviceSegmentedReduce::Reduce(nullptr,
                                                     ...
                                                     row_inf_norm.data(),
                                                     n_rows,
                                                     op_problem_scaled_.offsets.data(),
                                                     op_problem_scaled_.offsets.data() + 1,
                                                     max_op_t<f_t>{},
                                                     f_t(0),
                                                     stream_view_));
    ...
    auto coeff_nonzero_min_iter = thrust::make_transform_iterator(
      op_problem_scaled_.coefficients.data(), nonzero_abs_or_inf_transform_t<f_t>{});
    RAFT_CUDA_TRY(cub::DeviceSegmentedReduce::Reduce(nullptr,
                                                     current_required_bytes,
                                                     coeff_nonzero_min_iter,
                                                     row_min_nonzero.data(),
    ...
    auto coeff_nonzero_count_iter = thrust::make_transform_iterator(
      op_problem_scaled_.coefficients.data(), nonzero_count_transform_t<i_t, f_t>{});
    RAFT_CUDA_TRY(cub::DeviceSegmentedReduce::Reduce(nullptr,
                                                     current_required_bytes,
                                                     coeff_nonzero_count_iter,
                                                     row_nonzero_count.data(),
  ...
  if (nnz > 0) {
    compute_big_m_skip_rows_with_cub(op_problem_scaled_,
                                     temp_storage,
                                     temp_storage_bytes,
                                     row_inf_norm,
                                     row_min_nonzero,
    ...
    thrust::fill(
      handle_ptr_->get_thrust_policy(), row_inf_norm.begin(), row_inf_norm.end(), f_t(0));
    thrust::fill(
  ...
  for (int iteration = 0; iteration < row_scaling_num_iterations; ++iteration) {
    ...
    const double log2_sum = thrust::transform_reduce(
      handle_ptr_->get_thrust_policy(),
      ...
      [] __device__(auto row_info) -> double {
        ...
        if (skip_row || row_norm <= f_t(0)) { return 0.0; }
        return ::log2(static_cast<double>(row_norm));
      },
      0.0,
      thrust::plus<double>{});
    const i_t active_row_count = thrust::transform_reduce(
      handle_ptr_->get_thrust_policy(),
      thrust::make_zip_iterator(thrust::make_tuple(row_inf_norm.begin(), row_skip_scaling.begin())),
      thrust::make_zip_iterator(thrust::make_tuple(row_inf_norm.end(), row_skip_scaling.end())),
      [] __device__(auto row_info) -> i_t {
        const f_t row_norm = thrust::get<0>(row_info);
        const i_t skip_row = thrust::get<1>(row_info);
        return (!skip_row && row_norm > f_t(0)) ? i_t(1) : i_t(0);
      },
      i_t(0),
      thrust::plus<i_t>{});
    ...
    thrust::transform(
      ...
      [target_norm] __device__(auto row_info) -> f_t {
        const f_t row_norm = thrust::get<0>(row_info);
        const i_t skip_row = thrust::get<1>(row_info);
        if (skip_row || row_norm <= f_t(0) || target_norm <= f_t(0)) { return f_t(1); }
    ...
    // Delay A^T updates until after all row-scaling iterations are complete.
    // Rebuilding transpose from the final scaled A guarantees exact consistency for
    // downstream checks (e.g., concurrent root PDLP path).
  }
